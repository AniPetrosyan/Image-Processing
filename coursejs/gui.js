...
Gui.init = function(controlsChangeCallback) {
  let genFuncAddFilter = function(filterDef) {
    return function() {
  ...
  for (let filterIdx = 0; filterIdx < this.filterDefs.length; filterIdx++) {
    let filterDef = this.filterDefs[filterIdx];
    ...
    let filterButton = filterFolder.add(this.applyFuncs, filterDef.name);
  }
...
Gui.addHistoryEntry = function(filterDef, argVals) {
  ...
  for (let paramIdx = 0; paramIdx < filterDef.paramDefs.length; paramIdx++) {
    let paramDef = filterDef.paramDefs[paramIdx];
...
// ogtvel em http://stackoverflow.com/questions/1458633/how-to-deal-with-floating-point-number-precision-in-javascript
